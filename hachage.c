...
void *hashmap_get(HashMap *map, const char *key){
    ...
    for(int i=0;i<map->size;i++){
        unsigned long curIndex =probing_lineaire(cleHachee,i);
        if(map->table[curIndex].value == NULL){return NULL;} // Comme le probing est linéaire, alors si on ne trouve aucun element au début c'est que l'element n'est pas présent
        if(map->table[curIndex].value == TOMBSTONE){
            continue; // On continue le probing
        }
        if(strcmp(map->table[curIndex].key,key)==0){return map->table[curIndex].value;}
    }
    return NULL;
}
...
int hashmap_remove(HashMap *map, const char *key){
    ...
    for(int i=0;i<map->size;i++){
        unsigned long curIndex =probing_lineaire(cleHachee,i);
        if(map->table[curIndex].key ==NULL){return -1;}//Pas dans la table
        if(map->table[curIndex].value == TOMBSTONE){
            continue; // On continue le probing
        }
        if(strcmp(map->table[curIndex].key,key)==0){
